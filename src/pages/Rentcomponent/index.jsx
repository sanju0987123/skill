    ...
    const fetchUsersData = async () => {
      try {
        ...
        const rentUsersData = response.documents.filter(
          user => user.employeementstatus === 'Rent' && user.email && user.email.trim() !== ''
        );
      ...
      <div className={`flex flex-col sm:flex-row gap-10 items-center justify-between max-w-[1402px] mt-8 mx-auto md:px-5 w-full `}>
        ...
        <div className="grid grid-cols-5 gap-9 max-w-[1402px] mt-2.5 mx-auto md:grid-cols-3 sm:flex-col flex-row justify-center">
          ...
          {uniqueUsersData.map((user, index) => (
            <div key={`${user.email}_${index}`} className="mb-8    box-border p-4 shadow-lg hover  hover hover:shadow-xl hover:bg-gray-300">
              {/* TailwindCard */}
              <div>
                <TailwindCard
              ...
              <div>
                <Text
                  key={index}
                  className="m-auto text-[22px] text-gray-900 sm:text-lg md:text-xl overflow-x-hidden cursor-pointer ml-2"
                ...
                >
                  ...
                  <span className="text-gray-900 font-quicksand mt-1 text-left text-xl font-medium overflow-x-hidden ">
                    {user.abooutyou} {/* Typo in 'aboutyou'? */}
                    <br />
                  </span>
                  <span className="text-gray-900 font-quicksand ml-2 text-left text-xl font-medium">
                    {user.role}
                  </span>
                </Text>
              </div>
